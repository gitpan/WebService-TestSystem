...
sub new {
    ...
    # Note that we do not accept configuration parameters from the new()
    # call, because any SOAP user can call new(), so we cannot trust the
    ...
    my %config;
    my $confname = "/etc/webservice_testsystem/testsystem.conf";
    if (!Config::Simple->import_from($confname,\%config)) {
        $self->{_error_msg} = "$confname: ".Config::Simple->error()."\n";
    }
    foreach my $param (qw(stpdb_dbi stpdb_user stpdb_pass)) {
        if (defined $config{$param}) {
            $self->{"_$param"} = $config{$param};
        }
    }
...
sub _get_dbh {
    ...
    $self->{'_stpdb_dbh'} = 
        DBI->connect_cached($self->{'_stpdb_dbi'}, 
                            $self->{'_stpdb_user'}, 
                            $self->{'_stpdb_pass'}
                            );
    if (! $self->{'_stpdb_dbh'}) {
...
sub _metrics {
    ...
    if (! defined $self->{_metrics}) {
        $self->{_metrics} = new WebService::TestSystem::Metrics(app => $self);
    }
...
# TODO:  Can these all be autogenerated?
...
################
# Eventually, everything after this line should be moved into sub objects
# and auto-dispatched
...
# environment_default, lilo_default, repeat_safe

sub get_tests {
    my $self = shift;

    my $sql = qq|SELECT uid,descriptor, description, category, code_location, configuration_notes,status from test|;
...
record will include:  uid, descriptor, host_type_uid, host_state_uid
...
sub get_hosts {
    ...
    my $sql = qq|SELECT uid, descriptor, host_type_uid, host_state_uid from host|;
    my $dbh = $self->_get_dbh() or return undef;
    my $sth = $dbh->prepare($sql);
    $sth->execute;
...
the system.  Each image record includes its descriptor, uid, and status.
...
sub get_images {
    ...
    my $sql = qq|SELECT uid,descriptor,status FROM distro_tag WHERE status='Available'|;
    my $dbh = $self->_get_dbh() or return undef;
    my $sth = $dbh->prepare($sql);
    $sth->execute;
